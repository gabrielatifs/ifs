...
import { UserProvider, useUser } from '@ifs/shared/components/providers/UserProvider';
import { AdminModeProvider } from '@ifs/shared/components/providers/AdminModeProvider';
import { PostHogProvider, usePostHog } from '@ifs/shared/components/providers/PostHogProvider';
import { QueryProvider } from '@ifs/shared/components/providers/QueryProvider';
...
function LayoutContent({ children, currentPageName }) {
  ...
  useEffect(() => {
    ...
    if (isPostHogReady) {
      trackPage(currentPageName, {
        path: pathname,
        is_admin_page: adminPages.includes(currentPageName) && !isPublicEventView
      });
    }
  ...
  useEffect(() => {
    ...
    const isAdminPage = adminPages.includes(currentPageName) && !isPublicEventView;
    if (!isAdminPage) {
      initApolloTracking();
    }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (metaRobots && isAdminPage) {
        metaRobots.content = 'index, follow';
      }
    };
  }, [currentPageName, isPublicEventView]);

  // Portal app - all pages use portal layout
  if (noLayoutPages.includes(currentPageName)) {
    console.log(`[DEBUG] Page '${currentPageName}' is in noLayoutPages. Rendering without layout.`);
    return (
      <PageErrorBoundary>
        {children}
        <AdminModeToggle />
  ...
  // All other pages use portal layout
