...
import { ArrowRight, Bell, X, Shield, Building2, Loader2, Play, Crown, Gift, Copy } from 'lucide-react';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        if (!user.onboardingCompleted && !user.isUnclaimed && !isFreshLogin) {
    ...
    useEffect(() => {
        ...
        // Check for referral code for existing users
        if (refCode && user && !user.hasUsedReferral && user.membershipType === 'Associate') {
    ...
    const handleAcceptReferral = async () => {
        try {
            ...
            toast({
                ...
                description: "Upgrade to Full Membership to claim your first month free.",
                duration: 5000
            });
            ...
            toast({
                title: "Error",
                description: "Failed to apply referral. Please try again.",
                variant: "destructive"
            });
        }
    };
    ...
    const copyReferralCode = () => {
        const code = userReferralCode || user?.referralCode;
        if (!code) return;
                    ...
                    <div className="max-w-7xl mx-auto w-full space-y-8">
                        ...
                        <div className="grid lg:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Referral CTA */}
                                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg border-2 border-purple-200 overflow-hidden">
                                    <div className="p-5">
                                        <div className="flex items-start gap-3 mb-4">
                                            ...
                                            <div className="flex-1">
                                                <h3 className="font-semibold text-purple-900 mb-1">Refer & Get Free Months</h3>
                                                <p className="text-sm text-purple-700 leading-relaxed">
                                                    Both you and your friend get 1 free month when they join!
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex gap-2 mb-3">
                                            <input
                                                type="text"
                                                value={userReferralCode || user?.referralCode || 'Loading...'}
                                                readOnly
                                                className="flex-1 px-3 py-2 text-sm font-mono font-bold bg-white border border-purple-200 rounded-md text-purple-900"
                                            />
                                            <Button
                                            ...
                                            >
                                                {copiedReferral ? <span className="text-green-600">✓</span> : <Copy className="w-4 h-4" />}
                                            </Button>
                                        </div>
                                        <Link to={createPageUrl('MyProfile') + '?tab=referrals'}>
